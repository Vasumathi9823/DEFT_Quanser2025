...
pick_up=0
person_flag=0
traffic_light_direction=[300,620]   # look for traffic light with x between 300 and 620
traffic_stop=0
...
nodeSequence = [10 , 2,4,14,20,22,9,0,2,4,6,8,10]
...
class SteeringController:
    ...
    def update(self, p, th, speed, dt,k):
        self.k=k
        global done, nextSlope, previousSlopes, waypointCounter,slope_array, traffic_light_direction
        ...
        if self.wpi<3000 and self.wpi+90<self.N:
        ...
        if abs(slope)<10:
            traffic_light_direction=[300,620] # look for traffic light with x between 300 and 620 ie center
        
        elif slope>15:
            traffic_light_direction=[0,300] # look for traffic light with x between 300 ie left

        elif slope< -15:
            traffic_light_direction=[620,820] # look for traffic light with x between 300 and 620
...
#-------Seperate Thread for yolo detection----------- 
def yolo_detection():
...
# Class best index names: ['green', 'person', 'red', 'round', 'stop', 'yellow', 'yield']
# best2 {0: 'cone', 1: 'green', 2: 'person', 3: 'red', 4: 'round', 5: 'stop', 6: 'yellow', 7: 'yield'}
                        ...
                        for result in results:
                            ...
                            for box in boxes:
                                ...
                                if cls==5 and stop_sign==0:
                                    ...
                                    # print(f"the image h,w are {x1-x2},{y1-y2}")
                                            # Confidence score
                                    if abs(x1-x2)>55:
                                        stop_sign=1
                                

                                if (cls!=1 or cls!=3 or cls!=6) and (x2< traffic_light_direction[0]) or (x1>traffic_light_direction[1]):
                                    traffic_stop=0
                                    continue
                            
                                if cls==3 and traffic_stop==0 :  #red
                                    # x1, y1, x2, y2 = map(int, box.xyxy[0])  # Bounding box coordinates
                                    # h,w=abs(x1-x2),abs(y1-y2)

                                    if  w>13:
                                        traffic_stop=1
                                        print(f"red_light {traffic_light_direction}")

                                elif cls==6 and traffic_stop==1 :  # yellow
                                    # x1, y1, x2, y2 = map(int, box.xyxy[0])  # Bounding box coordinates
                                    

                                    if  w>13:
                                        traffic_stop=0
                                        print(f"yellow_light {traffic_light_direction}")


                                elif cls==1 and traffic_stop==1:   # green
                                    # x1, y1, x2, y2 = map(int, box.xyxy[0])  # Bounding box coordinates
            
                                    if  w>13 :
                                        traffic_stop=0
                                        print(f"green_light {traffic_light_direction}")
                            
                            if 2 in res_array:
                                x1,x2,y1,y2=res_array[0]
                                w=abs(x1-x2)
                                
                                if  w>28:   #((x1+x2)/2)>100 and ((x1+x2)/2)<600 and
                                    person_flag=1

                                else:
                                    # print("person detected not stopping")
                                    person_flag=0
                            else:
                                person_flag=0
            except IndexError:
...
def controlLoop():
    ...
    with qcar, gps:
        ...
        while (t < tf+startDelay) and (not KILL_THREAD):
            ...
            # drop_off_norm=np.linalg.norm(np.subtract([-0.905,0.800],[x,y]))
            # taxi_hub_norm = np.linalg.norm(np.subtract([-0.733, -0.969],[x,y]))
            pick_up_norm=np.linalg.norm(np.subtract([0.125, 4.395],[x,y]))
